...
let timeLeft = 10;
let timerInterval;
let totalTime = 10; // Total time for countdown

// Pick 5 random questions from the array
const selectedQuizData = quizData.sort(() => 0.5 - Math.random()).slice(0, 5);
...
function loadQuiz() {
  ...
  // Add the current question index before the question text
...
function startTimer() {
  ...
  secondHalf.style.transform = 'rotate(0deg)'; // Reset circular progress
  ...
  timerInterval = setInterval(() => {
    ...
    const progressAngle = (1 - timeLeft / totalTime) * 360;
    secondHalf.style.transform = `rotate(${progressAngle}deg)`;
...
submitBtn.addEventListener("click", () => {
  ...
  if (answer) {
    ...
    clearInterval(timerInterval); // Stop timer when user submits an answer
